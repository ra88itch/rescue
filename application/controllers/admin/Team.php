...
class Team extends CI_Controller {
    ...
    function __construct() {
		...
		$this->load->model(array('systemModel','teamModel'), '', TRUE);

		/*$this->load->model('user_model', '', TRUE);
		$user = $this->user_model->get_user_cookie();
		if(isset($user) && $user->level > '1'){
			//redirect('cms/permissionRequest');
		}*/
    }
    ...
    public function index() {
		...
		if($this->input->post()){
			...
			if($username == '' || $password == ''){
				...
				$user = $this->teamModel->auth($username, $password);
				if($user == false){
		...
		$response['errorDesc'] = $this->systemModel->getErrorDesc($response['errorCode']);

		$this->load->view('cms/login', $response);
	}
