<?php
$image_path = 'data:image/png;base64,';

if(isset($detail)){
	if($detail->image != ''){
		$image_path = $root.'assets/uploads/'.$detail->image;
	}
}
?>
<div id="profile-basic" class="tab-pane fade in active row">
	<div class="col-xs-12 col-md-10 col-md-offset-1 col-lg-8 col-lg-offset-2">
		<div class="row">

			<div class="col-xs-12 col-sm-6 col-md-3">

				<div class="imageBox"style="background: #fff url('<?php echo $image_path; ?>') no-repeat center center / cover;">
					<div class="thumbBox"></div>
					<div class="btn-browse-image"><i class="pe-7s-camera"></i></div>
				</div>
				<div class="crop-action">
					<input type="file" id="file" style="float:left; width: 250px; display:none;">
					<input type="button" id="btnZoomIn" value="+" style="float: right; width:50%;">
					<input type="button" id="btnZoomOut" value="-" style="float: right; width:50%;">
					<input type="hidden" id="image-data" name="image-data" value="">
				</div>

			</div>

			<div class="col-xs-12 col-sm-6 col-md-9">
				<div class="row">

					<div class="col-xs-12 col-sm-12 col-md-7">
						<div class="form-group">
							<label for="title">คำนำหน้า<span class="text-danger"> *</span></label>
							<input type="text" class="form-control" name="title" placeholder="คำนำหน้า" value="<?php 
									if(isset($detail)){ 
										echo $detail->title; 
									} ?>" required>
						</div>
						<div class="form-group">
							<label for="firstname">ชื่อ<span class="text-danger"> *</span></label>
							<input type="text" class="form-control" name="firstname" placeholder="ชื่อ" value="<?php 
									if(isset($detail)){ 
										echo $detail->firstname; 
									} ?>" required>	
						</div>
						<div class="form-group">
							<label for="lastname">นามสกุล<span class="text-danger"> *</span></label>
							<input type="text" class="form-control" name="lastname" placeholder="นามสกุล" value="<?php 
									if(isset($detail)){ 
										echo $detail->lastname; 
									} ?>" required>	
						</div>
						<div class="form-group">
							<label for="nickname">ชื่อเล่น<span class="text-danger"> *</span></label>
							<input type="text" class="form-control" name="nickname" placeholder="ชื่อเล่น" value="<?php 
									if(isset($detail)){ 
										echo $detail->nickname; 
									} ?>" required>	
						</div>
						<div class="form-group">
							<label for="thai_id">เลขประจำตัวบัตรประชาชน<span class="text-danger"> *</span></label>
							<input type="text" class="form-control" name="thai_id" placeholder="เลขประจำตัวบัตรประชาชน 13 หลัก" value="<?php 
									if(isset($detail)){ 
										echo $detail->thai_id; 
									} ?>" required maxlength="13" minlength="13">	
						</div>
						<div class="form-group">
							<label for="dob">วัน/เดือน/ปี เกิด (ค.ศ.)<span class="text-danger"> *</span></label>
							<input type="text" class="form-control" name="dob" placeholder="วัน/เดือน/ปี เกิด" value="<?php 
									if(isset($detail)){ 
										echo $detail->dob; 
									} ?>" required>	
						</div>
					</div>

					<div class="col-xs-12 col-sm-12 col-md-5">	
						<div class="form-group">
							<label for="code">รหัสประจำตัว</label>
							<input type="text" class="form-control" name="code" placeholder="สัญชาติ" value="<?php 
									if(isset($detail)){ 
										echo $detail->code; 
									} ?>">	
						</div>
						<div class="form-group">
							<label for="religion">หมู่โลหิต</label>
							<select class="form-control" name="blood_type">
							<?php
							$option = '';
							foreach($blood AS $value_blood){
								$option_status = '';
								if(isset($detail) && $detail->blood_type == $value_blood->id){
									$option_status = ' selected';
								}
								$option .= '<option value="'.$value_blood->id.'"'.$option_status.'>'.$value_blood->name.'</option>';
							}
							echo $option;
							?>
							</select>
						</div>									
						<div class="form-group">
							<label for="status">สถานะ</label>
							<select class="form-control" name="status">
							<?php
							$option = '';
							foreach($status AS $value_status){
								$option_status = '';
								if(isset($detail) && $detail->status == $value_status->id){
									$option_status = ' selected';
								}
								$option .= '<option value="'.$value_status->id.'"'.$option_status.'>'.$value_status->name.'</option>';
							}
							echo $option;
							?>
							</select>
						</div>
						<div class="form-group">
							<label for="origin">เชื้อชาติ<span class="text-danger"> *</span></label>
							<input type="text" class="form-control" name="origin" placeholder="เชื้อชาติ" value="<?php 
									if(isset($detail)){ 
										echo $detail->origin; 
									} ?>" required>	
						</div>
						<div class="form-group">
							<label for="nationality">สัญชาติ<span class="text-danger"> *</span></label>
							<input type="text" class="form-control" name="nationality" placeholder="สัญชาติ" value="<?php 
									if(isset($detail)){ 
										echo $detail->nationality; 
									} ?>" required>	
						</div>
						<div class="form-group">
							<label for="religion">ศาสนา</label>
							<select class="form-control" name="religion">
							<?php
							$option = '';
							foreach($religion AS $value_religion){
								$option_status = '';
								if(isset($detail) && $detail->religion == $value_religion->id){
									$option_status = ' selected';
								}
								$option .= '<option value="'.$value_religion->id.'"'.$option_status.'>'.$value_religion->name.'</option>';
							}
							echo $option;
							?>
							</select>
						</div>
					</div>

				</div>
			</div>

		</div>

	</div>
</div>