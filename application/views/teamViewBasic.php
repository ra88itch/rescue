...
if(isset($detail)){
	if($detail->thai_id!=''){
		$thai_id = str_split($detail->thai_id);
		$thai_id = $thai_id[0]. '-'. $thai_id[1]. $thai_id[2]. $thai_id[3]. $thai_id[4]. '-'. $thai_id[5]. $thai_id[6]. $thai_id[7]. $thai_id[8]. $thai_id[9]. '-'. $thai_id[10]. $thai_id[11]. '-'. $thai_id[12];
	}
	$profile->status		= $status[$detail->status];
	$profile->blood			= $blood[$detail->blood_type];
	$profile->religion		= $religion[$detail->religion];

	if($detail->dob != ''){
		$current_year = DATE('Y');
		$birth_year = DATE('Y', strtotime($detail->dob));

		$profile->dob   = DATE('d/m/', strtotime($detail->dob));
		$profile->dob   .= $birth_year + 543;
		
		$age = $current_year - $birth_year;	
		$profile->dob   .= ' <span class="small">['.$age.' ปี]</span>';
	}
				...
				<div class="row">
					...
					<div class="col-xs-12 col-sm-12 col-md-7">
						<div class="form-group">
							...
							<h5><?php echo $detail->title.$detail->firstname,' ',$detail->lastname,' <span class="small">[',$detail->nickname,']</span>'; ?></h5>
						</div>
						<div class="form-group">
							<label for="title">เลขประจำตัวบัตรประชาชน</label>
							<h5><?php echo $thai_id; ?></h5>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<label for="title">เชื้อชาติ/สัญชาติ/ศาสนา</label>
							<h5><?php echo $detail->origin,'/',$detail->nationality,'/',$profile->religion; ?></h5>
						</div>
					</div>
					...
					<div class="col-xs-12 col-sm-12 col-md-5">	
						...
						<div class="form-group">
							<label for="title">โทรด่วน</label>
							<a class="btn btn-block btn-<?php if($hotline=='#'){ echo 'danger disabled'; }else{ echo 'success  btn-fill'; } ?>" href="<?php echo $hotline; ?>"><i class="pe-7s-call"></i> <?php echo $detail->hotline; ?></a>
						</div>
					</div>
